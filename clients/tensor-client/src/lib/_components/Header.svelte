  ...
  const on_file_upload = (e: InputEvent) => {
    ...
    for (const file of input.files!) {
      ...
      fileReader.onload = (event) => {
        ...
        if (!!data) {
          const parsed_data = client_config_validator.parse(
...
<header
  class="relative p-2 isolate bg-amber-400/10 backdrop-blur-lg flex justify-between items-center self-start w-full"
>
  <a href="/" class="p-2 bg-amber-500/20 rounded-md">
    <img src="./vite.svg" alt="Logo" />
  </a>
  <Button class="p-2 flex flex-col gap-1 rounded-md relative">
    <Label for="ConfigUpload" class="flex gap-2 items-center"
      ><Upload /> Upload a new Config</Label
    >
    <Input
      on:input={on_file_upload}
      id="ConfigUpload"
      type="file"
      class="absolute inset-0 opacity-0"
      accept=".conf, application/json"
      multiple
    />
  </Button>
</header>
