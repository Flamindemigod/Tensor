...
import * as Dialog from "$lib/components/ui/dialog";
  ...
  const file_handler =  async ()=>{
      ...
      files.set(await Promise.all(data.map(async (d)=>await blobToBase64(d))));
    }};
  onMount(()=>{
    window.addEventListener('paste', e => {
    if (!!e.clipboardData?.files)
      image_upload.files = e.clipboardData.files;
      file_handler();
});
  }) 
</script>
...
>
  {#each $SERVERS as server}
        <Dialog.Root onOpenChange={(open)=>{ if (!open) context=null}}>
    <ContextMenu.Root>
      <Tooltip.Root>
        <Tooltip.Trigger>
          <ContextMenu.Trigger>
        ...
        <ContextMenu.Content class="grid grid-cols-[32px_1fr]">
          <ContextMenu.Item class="col-span-full grid grid-cols-subgrid"
            >
          <Dialog.Trigger on:click={()=>{context = "ServerIcon"}} class="col-span-full grid grid-cols-subgrid justify-start">
              <Image />
      ...
      <Dialog.Description> 
      <Input on:change={file_handler} bind:ref={image_upload} type="file" id="img" name="img" accept="image/*" />
      ...
      {#if !!$files.length}
                <DialogClose>
      <Button on:click={()=>{setServerIcon(server, $files.at(0))}}>Use as New Server Icon</Button> 
                </DialogClose>
      {/if}
      </Dialog.Description>
    </Dialog.Header>
